...
public:
    ...
    //     // Initialize Phong color components
    ...
    //         double diffuseIntensity = std::max(0.0, hitRecord.normal.dot((light->getPosition() - hitRecord.hit_point).normalize()));
    //         diffuseColor = diffuseColor + light->getIntensity() * material->kd * material->diffusecolor * diffuseIntensity;

    //         // Specular reflection
    //         Vec3 viewDirection = -ray.getDirection().normalize();
    //         Vec3 reflectedLight = material->reflect((light->getPosition() - hitRecord.hit_point).normalize(), hitRecord.normal);
    //         double specularIntensity = std::pow(std::max(0.0, viewDirection.dot(reflectedLight)), material->specularexponent);
    //         specularColor = specularColor + light->getIntensity() * material->ks * material->specularcolor * specularIntensity;
    //     }
